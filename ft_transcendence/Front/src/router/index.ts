...
async function renderProtectedRoute(renderFunction: () => void | Promise<void>) {
  //console.logog('🔐 Verificando autenticação para rota protegida...');
  
  const isAuthenticated = await validateAndCleanAuthState();
  
  if (!isAuthenticated) {
    //console.logog('❌ Usuário não autenticado - redirecionando para login');
  ...
  //console.logog('✅ Usuário autenticado - renderizando rota protegida');

  ...
  try {
    ...
    if (!profileResponse.ok) {
      if (profileResponse.status === 401 || profileResponse.status === 403) {
        //console.logog('❌ Sessão inválida na rota protegida - redirecionando para home');
    ...
    //console.logog("Dados do perfil:", profileData);
...
// Busca dados de ranking (wins, losses, score)
    ...
    try {
      //console.logog("ID do usuário que será enviado:", profileData.id);
      ...
      //console.logog("Status da resposta ranking:", rankingResponse.status);
      
      if (rankingResponse.ok) {
        const rankingData = await rankingResponse.json();
        //console.logog("Dados de ranking recebidos:", rankingData);
    ...
    if (error instanceof Error && error.message === 'Sessão expirada') {
      //console.logog('🔄 Sessão expirada detectada - redirecionando para home');
      window.location.href = '/';
      return null;
    }
    
    // Para outros erros, também redireciona
    //console.logog('🔄 Erro de autenticação - redirecionando para home');
    window.location.href = '/';
    return null;
  }
}
    ...
    if (value.length >= 2) {
      try {
        ...
        if (response.ok) {
          ...
          //console.logog('Usuários encontrados:', users);

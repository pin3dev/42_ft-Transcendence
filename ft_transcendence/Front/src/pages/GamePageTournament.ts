...
export function GamePageTournament(): void {
  //console.logog("Iniciando renderização da GamePageTournament..."); // LOG 1
  ...
  //console.logog("LOG 2: Containers das tabelas encontrados:", { rankingTableBody, matchListContainer });

  // --- CALLBACKS PARA ATUALIZAÇÃO DA UI ---

  const handleRankingUpdate =  async (rankingsFromSocket: any[]) => {
    //console.logog("LOG 4a: handleRankingUpdate ACIONADO com:", rankingsFromSocket);
    ...
    try {
        ...
        //console.logog("LOG 5a: Nomes do ranking buscados. Dados formatados:", formattedRankings);

        // Atualiza a tabela com os dados completos
        updateRankingTable(rankingTableBody, formattedRankings);
        //console.logog("LOG 6a: Tabela de Ranking ATUALIZADA.");
  ...
  const handleTournamentUpdate = async (matchesFromSocket: any[]) => {
    //console.logog("LOG 4b: handleTournamentUpdate ACIONADO com:", matchesFromSocket);
    ...
    try {
        ...
        //console.logog("LOG 5b: Nomes buscados com sucesso. Dados formatados:", formattedMatches);

        updateMatchSchedule(matchListContainer, formattedMatches);
        //console.logog("LOG 6b: Tabela de Partidas ATUALIZADA.");
  ...
  //console.logog("LOG 3: Chamando renderPongGameTournament com os callbacks.");
  ...
  cleanupCurrentPage = () => {
    //console.logog("Limpando GamePageTournament...");
    cleanupGame();
  };
}
...
export function renderPongGameTournament(
  ...
  matchmakingUI.className = `
        ...
        transition-opacity duration-300 gap-4`;
  ...
  // --- NOVO: UI para seleção de jogadores do torneio ---
  const tournamentSetupContainer = document.createElement('div');
  tournamentSetupContainer.id = 'tournament-setup';
  // Inicialmente visível. Esconderemos após a criação do torneio.
  tournamentSetupContainer.className = 'w-full flex flex-col items-center gap-4 text-white';

  const tournamentTitle = document.createElement('h3');
  tournamentTitle.className = 'text-2xl font-bold';
  tournamentTitle.textContent = 'Criar Torneio';

  const sliderLabel = document.createElement('label');
  sliderLabel.htmlFor = 'player-slider';
  sliderLabel.className = 'text-lg';
  sliderLabel.textContent = 'Jogadores: 2'; // Valor inicial

  const playerSlider = document.createElement('input');
  playerSlider.type = 'range';
  playerSlider.id = 'player-slider';
  playerSlider.min = '2';
  playerSlider.max = '16';
  // A API de torneio retorna erro para números ímpares, então forçamos passos de 2.
  playerSlider.step = '2';
  playerSlider.value = '2';
  playerSlider.className = 'w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer';

  const createTournamentButton = document.createElement('button');
  createTournamentButton.id = 'create-tournament-button';
  createTournamentButton.className = 'mt-2 px-8 py-3 bg-neon-blue text-white font-bold rounded-lg hover:bg-opacity-80 transform hover:scale-105 transition-all duration-200';
  createTournamentButton.textContent = 'Criar Torneio';

  tournamentSetupContainer.append(tournamentTitle, sliderLabel, playerSlider, createTournamentButton);
  ...
  joinButton.id = 'join-button'; // ID mais genérico
  ...
  // --- NOVO: Listeners para a UI do torneio ---
  let selectedPlayers = 2; // Guarda o número de jogadores do slider

  playerSlider.addEventListener('input', () => {
    selectedPlayers = parseInt(playerSlider.value, 10);
    sliderLabel.textContent = `Jogadores: ${selectedPlayers}`;
  });

  const handleCreateTournamentClick = () => {
    ws?.send(JSON.stringify({ type: 'TOURNAMENT_CREATE', value: selectedPlayers }));
  ...
  const handleJoinTournamentClick = () => {
    ...
    joinButton.classList.add('hidden'); // Esconde o botão após clicar

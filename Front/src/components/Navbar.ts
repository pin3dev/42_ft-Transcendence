...
function showLogoutModal(): void {
  ...
  logoutModal.querySelector('#confirm-logout')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (response.ok) {
        //console.logog('Logout realizado com sucesso no servidor');
      } else {
      ...
      //console.logog('🔌 Desconectando socket de status durante logout');
    ...
    if (isDropdownOpen && dropdownElement) {
      ...
      menuItems.forEach(item => {
        ...
        link.addEventListener('click', async (e) => {
          e.preventDefault(); 
          //console.logog(`Ação do menu: ${item.action}`);
...
export function createNavbar(): HTMLElement {
  ...
  if (!isUserAuthenticated) {
    //console.logog('👤 Exibindo botões de Login/Registrar');
    ...
    //console.logog('⚙️ Exibindo menu de configurações');

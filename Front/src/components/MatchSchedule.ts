...
const initialScheduleData: MatchData[] = [
    { id: 1, player1Name: 'NeonMaster99', player2Name: 'PaddleWizard', score1: 5, score2: 2 },
    { id: 2, player1Name: 'ArcadeLegend', player2Name: 'PingPongKing', score1: 3, score2: 5 },
    { id: 3, player1Name: 'Player 3', player2Name: 'Player 4', score1: 0, score2: 0 },
];
...
function createMatchBlock(match: MatchData): HTMLElement {
    ...
    scoreboardContainer.className = 'scoreboard flex justify-around items-center w-full max-w-[800px] text-white';
    ...
    score1Element.className = 'text-5xl font-bold font-mono text-neon-green';
    ...
    player2Info.className = 'flex items-center gap-4'; // Removed flex-row-reverse for consistency
...
export function updateMatchSchedule(container: HTMLElement, matches: MatchData[]): void {
    ...
    matches.forEach((match, index) => {
        ...
        if (index < matches.length - 1) {
            ...
            separator.className = 'h-px bg-neon-green/30 mx-4';
...
export function createMatchSchedule(): HTMLElement {
    ...
    mainTitle.className = 'text-3xl md:text-4xl font-bold mb-8 text-white text-center';
    mainTitle.textContent = 'PARTIDAS';
    container.appendChild(mainTitle);

    // The main "arcade box" container
    const arcadeContainer = document.createElement('div');
    arcadeContainer.className = 'arcade-container bg-arcade-darker rounded-lg overflow-hidden border-2 border-neon-green max-w-2xl mx-auto';

...
export function render2FAPage(qrCode?: string | null): void {
    ...
    if (qrCode) {
      // Exibe o QR Code para configuração inicial
      const qrImage = document.createElement('img');
      qrImage.src = qrCode; // Agora será uma URL base64
      ...
      // Exibe o campo para inserir o token diretamente
...
function render2FAInput(container?: HTMLElement): void {
    // Garante que o container seja um HTMLElement válido
    if (!container) {
      container = document.querySelector('.w-full.max-w-lg.text-center') as HTMLElement;
      if (!container) {
        throw new Error('Container não encontrado para renderizar o campo de 2FA.');
      }
      container.innerHTML = '';
    }
